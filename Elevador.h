...
public:
  int load_map(std::string &input_map, std::string &class_attribute) {
    
    // Prepare input map
    clock_t start_time = clock();
    GDALAllRegister();
    GDALDataset *input_map_dataset = (GDALDataset*) GDALOpenEx(input_map.c_str(), GDAL_OF_READONLY, NULL, NULL, NULL);
  ...
  //      std::cout << "Authority: " << authority << " code: " << code << std::endl;
      ...
      while ((input_feature = input_layer->GetNextFeature()) != NULL) {
        ...
        for (int current_field = 0; current_field < input_feature->GetFieldCount(); ++current_field) {
          if (strcmp(input_feature->GetFieldDefnRef(current_field)->GetNameRef(), class_attribute.c_str()) == 0) {
            cityjson_class = input_feature->GetFieldAsString(class_attribute.c_str());
            continue;
        ...
        if (wkbFlatten(input_feature->GetGeometryRef()->getGeometryType()) == wkbPolygon ||
          ...
          map.polygons.back().semantic_class = cityjson_class;
  ...
  int index_map() {
    ...
    for (std::vector<Polygon>::iterator current_polygon = map.polygons.begin(); current_polygon != map.polygons.end(); ++current_polygon) {
      edge_index.insert(current_polygon);
  ...
  //  edges_index.print_info();
    ...
    std::cout << "Indexed " << edge_index.size() << " half-edges edges of " << map.polygons.size() << " polygons in ";
    print_timer(start_time);
    std::cout << " using ";
    print_memory_usage();
    return 0;
  ...
  int load_point_cloud(std::string &input_point_cloud) {
    clock_t start_time = clock();
  ...
  int create_terrain_tin(std::set<std::string> &ignore_classes) {
    ...
    for (Kernel::FT x = point_cloud_index.x_min-0.5*dtm_search_radius; x < point_cloud_index.x_max+0.5*dtm_search_radius; x += dtm_cell_size) {
      for (Kernel::FT y = point_cloud_index.y_min-0.5*dtm_search_radius; y < point_cloud_index.y_max+0.5*dtm_search_radius; y += dtm_cell_size) {
        std::vector<Point_index *> intersected_nodes;
        point_cloud_index.find_intersections(intersected_nodes, x-0.5*dtm_search_radius, x+0.5*dtm_search_radius, y-0.5*dtm_search_radius, y+0.5*dtm_search_radius);
    ...
    for (Kernel::FT x = point_cloud_index.x_min-0.5*smoothing_radius; x < point_cloud_index.x_max+0.5*smoothing_radius; x += dtm_cell_size) {
      for (Kernel::FT y = point_cloud_index.y_min-0.5*smoothing_radius; y < point_cloud_index.y_max+0.5*smoothing_radius; y += dtm_cell_size) {
  ...
  int lift_flat_polygons(const char *cityjson_class, Kernel::FT ratio_to_use) {
    clock_t start_time = clock();
  ...
  int lift_polygon_vertices(const char *cityjson_class) {
    clock_t start_time = clock();
  ...
  int lift_polygons(const char *cityjson_class) {
    clock_t start_time = clock();
    std::size_t n_polygons = 0;
    for (auto &polygon: map.polygons) {
      if (polygon.semantic_class == cityjson_class) {
...
//                  std::cout << "\tdestination bottom: (" << destination_bottom << ")" << std::endl;
                  ...
                  if (result) {
                    if (const Kernel::Point_3 *intersection_point = boost::get<Kernel::Point_3>(&*result)) {
...
//                    } edge_index.edges[destination->point()].erase(origin->point());
  ...
  int write_3dcm_obj(std::string &output_3dcm) {
    clock_t start_time = clock();
  ...
  int write_3dcm_cityjson(std::string &output_3dcm) {
    clock_t start_time = clock();
  ...
  int write_terrain_obj(std::string &output_terrain) {
    clock_t start_time = clock();
    std::ofstream output_stream;
